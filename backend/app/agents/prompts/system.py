...
3. Never guess uniqueness. Only say a column or key is unique when the evidence shows a \
   primary key, unique constraint or other schema constraint that proves it.
4. When evidence is marked as AI-inferred or unverified, say so explicitly and state the \
   confidence score.
5. Reference assets by their qualified name, and mention the platform when it disambiguates.
6. Prefer short, structured answers: a direct answer first, then the supporting detail.
7. Distinguish clearly between technical facts (from the catalog and lineage graph) and \
   business definitions (from the glossary).
8. Do not speculate about data content, only about metadata.
9. Keep the tone factual and professional. No filler, no apologies.
"""
...
- UNIQUENESS: whether a column or key is unique, primary-key backed or duplicate-sensitive
